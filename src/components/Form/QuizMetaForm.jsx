...
export default function QuizMetaForm(props) {
	const { setDataFormQuizMeta } = props.formProps;
	...
	const onSubmit = (dataLog) => {
		...
		window.sessionStorage.setItem("ANSWER_SHEETS", JSON.stringify([]));
		window.sessionStorage.setItem("QUIZ_TIMER", dataLog.minute * 60);
		window.sessionStorage.setItem("QUIZ_CARD_INDEX", 0);
		window.sessionStorage.setItem("STATIC_TIME", dataLog.minute * 60);
