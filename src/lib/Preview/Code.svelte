...
<div>
  <div
    class="mx-auto text-white w-fit translate-y-px flex border-t border-r border-dashed border-white"
  >
    <button
      on:click={() => (lang = 'md')}
      class="bg-gray-800 px-2 py-1 border-l border-r border-dashed border-white text-sm font-bold"
      class:border-b={lang === 'html'}>Markdown</button
    >
    <button
      on:click={() => (lang = 'html')}
      class="bg-gray-800 px-2 py-1 border-r border-dashed text-sm font-bold"
      class:border-b={lang === 'md'}>HTML</button
    >
    <button
    ...
    >
      ...
      {:else}
        ...
        <span
          ...
          class="text-black absolute top-1/2 -translate-y-1/2 left-full ml-1 text-xs font-bold"
          >Copied!</span
        >
      {/if}
    </button>
  ...
  <pre
    class="px-4 py-2 bg-gray-800 text-white w-fit border border-dashed border-white max-w-[100vw] overflow-x-auto">{generatedCode}</pre>

  <div class="px-2 mt-2 flex items-center justify-center">
    <button
      class="{useLink
        ? 'bg-gray-800'
        : 'bg-gray-500'} text-white py-1 px-2 text-sm font-bold whitespace-nowrap"
